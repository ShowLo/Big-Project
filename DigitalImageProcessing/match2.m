...
for i = 1 : photoNum - 2
    ...
    for j = 1 : photoNum
        if(~added(j))                                               %只对于尚未加入序列的碎片做以下操作
            %用误差平方和的方法计算其他碎片的左边缘与待匹配右边缘的匹配程度
            LmatchR(j) = sum((left(:, j) - rightEdge).^2);
        else
            LmatchR(j) = Inf;
        end
    end
    for j = 1 : photoNum
        if(~added(j))                                               %只对于尚未加入序列的碎片做以下操作
            %用误差平方和的方法计算其他碎片的右边缘与待匹配左边缘的匹配程度
            RmatchL(j) = sum((right(:, j) - leftEdge).^2);
        else
            RmatchL(j) = Inf;
        end
    end

    [LRvalue, nextRight] = min(LmatchR);            %与待匹配右边缘匹配程度最大的左边缘
    [RLvalue, nextLeft] = min(RmatchL);              %与待匹配左边缘匹配程度最大的右边缘
